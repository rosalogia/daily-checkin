...
- [[#core-features][Core Features]]
  - [[#daily-check-in-system][Daily Check-in System]]
  - [[#user-registration--goal-management][User Registration & Goal Management]]
  - [[#streak-tracking][Streak Tracking]]
  - [[#administrative-controls][Administrative Controls]]
- [[#user-experience-flow][User Experience Flow]]
  - [[#registration][Registration]]
  - [[#daily-interaction][Daily Interaction]]
...
- [[#development-roadmap][Development Roadmap]]
  - [[#phase-1-foundation--basic-bot-setup][Phase 1: Foundation & Basic Bot Setup]]
  - [[#phase-2-core-user-management][Phase 2: Core User Management]]
  - [[#phase-3-admin-configuration][Phase 3: Admin Configuration]]
  - [[#phase-4-daily-posting-system][Phase 4: Daily Posting System]]
  - [[#phase-5-response-tracking--basic-streaks][Phase 5: Response Tracking & Basic Streaks]]
  - [[#phase-6-advanced-streak-logic][Phase 6: Advanced Streak Logic]]
...
- Posts automated daily messages in configured channels at configured times
- Pings all registered users to request progress updates
- Creates threads under daily posts for user responses (with date in thread name)
- Displays current streak counts for each user in the daily post
- 24-hour response window from daily post creation
...
** Streak Logic
- Count consecutive days with thread responses within 24-hour window
- Reset to 0 for missed days (unless 30+ day grace period applies)
- Grace period: Allow up to 2 missed days for streaks ≥30 days
- Automatic streak maintenance when daily posts are created
...
| ~/stats~               | View goal, streaks, and status (self or others)  | All users   |
| ~/set-checkin-channel~ | Configure daily post channel                     | Admin only  |
| ~/set-checkin-time~    | Configure daily post time & timezone             | Admin only  |
...
├── handler.rs           # Discord event handler with message processing
├── bot.rs               # Bot state management with shared data
├── data.rs              # Data structures and JSON persistence
├── scheduler.rs         # Daily posting and streak maintenance scheduler
├── streaks.rs           # Streak tracking and response validation logic
├── commands/
│   ├── mod.rs           # Command registration and routing
│   ├── ping.rs          # Basic ping command
│   ├── user.rs          # User commands (register/edit/deregister goals)
│   └── admin.rs         # Admin commands (set-checkin-channel/time)
└── utils/
...
- [X] Implement ~/edit-goal~ slash command (alias for register-goal)
- [X] Implement ~/deregister~ slash command
...
- [X] Smart re-registration (preserves data, resets streaks)
...
- [X] Implement ~/set-checkin-channel~ admin command
- [X] Per-server configuration storage
- [X] Permission checking for admin commands
- [X] Timezone configuration for servers
- [X] Implement ~/set-checkin-time~ admin command with timezone support

*Deliverable: Admins can configure where daily posts appear and when they are posted*

** Phase 4: Daily Posting System
*Goal: Bot automatically posts daily check-in messages*

- [X] Implement scheduled daily posting with cron-like functionality
- [X] Generate daily messages with user pings and user goals
- [X] Create threads automatically under daily posts
- [X] Optimized data storage (single daily post per guild)
- [X] Timezone-aware posting with configurable times
...
- [X] Monitor thread responses from registered users
- [X] Implement basic streak counting (consecutive days)
- [X] Simple streak reset on missed days
- [X] 24-hour response window enforcement
- [X] Grace period logic for 30+ day streaks
...
- [X] Implement 30+ day grace period logic
- [X] Handle edge cases (timezone differences, late responses)
