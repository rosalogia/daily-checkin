...
** Database Schema
- Users: Discord ID, server ID, goal text, current streak, last check-in date
- Servers: Server ID, configured channel, timezone setting
- Check-ins: User ID, date, thread message ID
...
- [ ] Set up Rust project with Discord library (serenity)
- [ ] Implement bot authentication and basic event handling
- [ ] Create basic SQLite database schema
- [ ] Implement ~/ping~ command to verify bot functionality
...
- [ ] Implement ~/register-goal~ slash command
- [ ] Implement ~/edit-goal~ slash command  
- [ ] Implement ~/deregister~ slash command
- [ ] Basic database operations (CRUD for users and goals)
