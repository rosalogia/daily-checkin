...
- [ ] =/stats= appears in slash command autocomplete
...
- [ ] Expect: Success response with welcome message
...
- [ ] Expect: Error response about empty goal

*** Test Case: Long Goal  
- [ ] Run =/register-goal= with goal text >500 characters
- [ ] Expect: Error response about character limit

*** Test Case: DM Usage (Should Fail)
- [ ] Try =/register-goal= in direct message to bot
- [ ] Expect: Error response about server-only usage
...
- [ ] Expect: ✅ success response mentioning "updated"
...
- [ ] Run =/deregister=
- [ ] Expect: ✅ success response mentioning streak count and re-registration option
...
- [ ] Expect: ❌ error response about not being registered
...
*** Test Case: View Own Stats (Registered User)
- [ ] Register: =/register-goal goal:"Test goal"=
- [ ] Run: =/stats=
- [ ] Expect: Discord embed displaying user stats
- [ ] Verify: Shows goal, current streak, longest streak, check-in status

*** Test Case: View Other User's Stats
- [ ] Have User A register a goal
- [ ] Run: =/stats user:@UserA= as different user
- [ ] Expect: Discord embed showing User A's stats
- [ ] Verify: Shows User A's goal and streak information

*** Test Case: Stats for Unregistered User
- [ ] Run: =/stats= without registering first
- [ ] Expect: Error message about not being registered
- [ ] Run: =/stats user:@UnregisteredUser=
- [ ] Expect: Error message about user not being registered
...
- [ ] Expect: Welcome message (treated as new registration)
...
- [ ] Expect: Welcome message with new goal
...
- Daily posting uses embeds and thread notifications for better user experience
- All features through Phase 6 have been implemented
